...
class SklearnClf:
    ...
    def get_avg_metrics_for_n_iterations(self, n_iterations=10, test_size=0.3, random_state=None):
        """
        ...
        """
        metrics_summary = {'precision': 0, 'recall': 0, 'f1_score': 0, 'confusion_matrix': np.zeros((2,2))}
        
        for _ in range(n_iterations):
            X_train, X_test, y_train, y_test = self.split_test_data(test_size, random_state)
            ...
            metrics_summary['confusion_matrix'] += confusion_matrix(y_test, self.predictions)
    ...
    def get_learning_curve(self):
        ...
        train_sizes, train_scores, validation_scores = learning_curve(
            self.clf, self.X, self.y, train_sizes=np.linspace(.01, 1.0, 10), cv=5, shuffle=True)
        ...
        ax.plot(train_sizes, validation_scores_mean, 'o-', label="Cross-validation score")
    ...
    def get_validation_curve(self, param_name="gamma", param_range=np.logspace(-6, -1, 5)):
        ...
        train_scores, test_scores = validation_curve(
            self.clf, self.X, self.y, param_name=param_name, param_range=param_range,
        ...
        ax.fill_between(param_range, np.min(train_scores, axis=1), np.max(train_scores, axis=1), alpha=0.2, color="darkorange", lw=2)
        ax.semilogx(param_range, test_scores_mean, label="Cross-validation score", color="navy")
        ax.fill_between(param_range, np.min(test_scores, axis=1), np.max(test_scores, axis=1), alpha=0.2, color="navy", lw=2)
